...
const AgricultureDatasetResultsPage = () => {
  ...
  const downloadOne = (ds) => {
    // <-- changed from ds.assets.data to ds.assets.ndvi
    const url = ds.assets?.ndvi;
    if (!url) return alert("No download URL available for this dataset.");
    window.open(
      outputFormat ? `${url}&format=${encodeURIComponent(outputFormat)}` : url,
      "_blank"
    );
  };
    ...
    try {
      const res = await fetch(`/api/agri/analyse`, {
        ...
        body: JSON.stringify({
          dataset_id: ds.id,
          analysis,
          // <-- use ndvi URL when POSTing back
          assets: {
            ndvi: ds.assets.ndvi,
            thumb: ds.assets.thumb || null, // ✅ fix: ensure thumb is included
          },
        }),
      });
      ...
      try {
        ...
        if (isQuotaError) {
          ...
          try {
            localStorage.setItem("analysisResults", JSON.stringify(evicted));
          } catch {}
        } else {
          throw e;
        }
      ...
      router.push("/agriculture-hotspot-dashboard");
    } catch (err) {
  ...
  const runAll = async () => {
    ...
    try {
      const jobs = datasets.map((ds) => ({
        dataset_id: ds.id,
        analysis: selected[ds.id] || "agriculture_hotspot",
        assets: {
          ndvi: ds.assets.ndvi,
          thumb: ds.assets.thumb || null, // ✅ fix: include thumb in each batch job
        },
      }));
            ...
            results: [
              {
                ...
                anomalies: payload.series.anomalies || [],
                breakpoints: payload.series.breakpoints || [],
              },
            ],
          })
        );
      } catch (e) {
        console.warn("Could not persist seriesResults", e);
      }
  ...
  const card = (d, i) => (
    <li
      ...
      className="card fade border border-green-300/50 dark:border-green-400/20 bg-white/80 dark:bg-slate-800/70 backdrop-blur-lg p-5 rounded-xl shadow-lg hover:shadow-xl hover:-translate-y-[2px] transition-all"
    >
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        {/* <select
          ...
          className="px-2.5 py-1 text-sm rounded-md border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900/40 focus:outline-none focus:ring-2 focus:ring-green-500"
        >
          <option value="">— select analysis —</option>
          <option value="agriculture_hotspot">
            Agriculture Hotspot Detection
          </option>
        </select> */}
        ...
        {!d.assets?.thumb && d.assets?.ndvi && (
          <a
            href={d.assets.ndvi}
            target="_blank"
            rel="noopener noreferrer"
            className="flex items-center gap-1 text-green-600 dark:text-green-400 text-xs hover:underline"
          >
  ...
  return (
    <main className="max-w-6xl mx-auto px-4 py-12">
      <h1 className="text-4xl sm:text-5xl font-extrabold mb-10 text-center bg-gradient-to-r from-green-600 to-lime-600 bg-clip-text text-transparent select-none">
          ...
          <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-6 mb-10">
            <label className="w-full sm:w-64">
              ...
              <select
                ...
                className="w-full px-3 py-2 rounded-md border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900/40 focus:outline-none focus:ring-2 focus:ring-green-500"
              >
            ...
            <button
              onClick={runAll}
              disabled={busy}
              className="relative inline-flex items-center justify-center gap-2 px-6 py-2 rounded-md font-medium text-white bg-gradient-to-r from-green-600 to-lime-600 hover:to-lime-700 active:scale-[.97] disabled:opacity-40 transition-transform"
            >
              {busy ? (
                <>
...
export default AgricultureDatasetResultsPage;
